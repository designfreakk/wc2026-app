"""Details page — all the technical breakdowns, kept off the simple front page."""
import pandas as pd
import streamlit as st

import engine

st.set_page_config(page_title="Details · WC2026", page_icon="📊", layout="wide")
st.title("📊 Details — under the hood")

R = st.session_state.get("results")
if R is None:
    st.info("Run a simulation on the **main page** first, then come back here.")
    st.stop()

_, _, _, gf, ks = engine.load_artifacts()
...
st.subheader("Knockout results (the sample bracket on the front page)")
ko = R["ko"]
